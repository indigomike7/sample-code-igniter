...
class User_ion_auth_model extends CI_Model
{
	/**
	...
	public function __construct()
	{
		...
		$this->tables  = $this->config->item('tables', 'user_ion_auth');

		//initialize data
		$this->identity_column = $this->config->item('identity', 'user_ion_auth');
		$this->store_salt      = $this->config->item('store_salt', 'user_ion_auth');
		$this->salt_length     = $this->config->item('salt_length', 'user_ion_auth');
		$this->join			   = $this->config->item('join', 'user_ion_auth');


		//initialize hash method options (Bcrypt)
		$this->hash_method = $this->config->item('hash_method', 'user_ion_auth');
		$this->default_rounds = $this->config->item('default_rounds', 'user_ion_auth');
		$this->random_rounds = $this->config->item('random_rounds', 'user_ion_auth');
		$this->min_rounds = $this->config->item('min_rounds', 'user_ion_auth');
		$this->max_rounds = $this->config->item('max_rounds', 'user_ion_auth');


		//initialize messages and error
		$this->messages = array();
		$this->errors = array();
		$this->message_start_delimiter = $this->config->item('message_start_delimiter', 'user_ion_auth');
		$this->message_end_delimiter   = $this->config->item('message_end_delimiter', 'user_ion_auth');
		$this->error_start_delimiter   = $this->config->item('error_start_delimiter', 'user_ion_auth');
		$this->error_end_delimiter     = $this->config->item('error_end_delimiter', 'user_ion_auth');
...
//                die($username);
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$manual_activation = $this->config->item('manual_activation', 'user_ion_auth');
...
//		$default_group = $this->where('name', $this->config->item('default_group', 'user_ion_auth'))->group()->row();
...
//
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login')
		                  ->where(''.$this->identity_column.' = \''.$this->db->escape_str($identity).'\' or email = \''.$this->db->escape_str($identity).'\'')
		                  ->limit(1)
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				if ($remember && $this->config->item('remember_users', 'user_ion_auth'))
				{
	...
	public function is_max_login_attempts_exceeded($identity) {
		if ($this->config->item('track_login_attempts', 'user_ion_auth')) {
			$max_attempts = $this->config->item('maximum_login_attempts', 'user_ion_auth');
			if ($max_attempts > 0) {
	...
	public function is_time_locked_out($identity) {
		...
		return $this->is_max_login_attempts_exceeded($identity) && $this->get_last_attempt_time($identity) > time() - $this->config->item('lockout_time', 'user_ion_auth');
	}
	 ...
	 * @author Ben Edmunds
	 **/
	public function group_id_from_name($name = NULL)
	{
	...
	**/
	...
	public function set_lang($lang = 'en')
	{
		...
		if($this->config->item('user_expire', 'user_ion_auth') === 0)
		{
		...
		{
			$expire = $this->config->item('user_expire', 'user_ion_auth');
		}
	...
	public function login_remembered_user()
	{
		...
		if ($query->num_rows() == 1)
		{
			...
			if ($this->config->item('user_extend_on_login', 'user_ion_auth'))
			{
				$this->remember_user($user->id);
			}
	...
	*/
	public function create_group($group_name = FALSE, $group_description = NULL)
	{
		...
		// bail if the group name already exists
                //echo $group_name;
                $query = $this->db->get_where('groups', array('name' => $group_name));
//                foreach ($query->result_array() as $row)
//                {
//                echo $row['id']." x ".$row["name"];
//                }
                
//                $existing_group = $query->row(); 
//                //echo $existing_group;
		if($query->num_rows()>0)
		{
...
//                echo $group_id."<br/>";
	...
	protected function _prepare_ip($ip_address) {
        ...
        function check_user_name_update($user_name, $id)
        {
            return count($this->db->query("select * from users where username ='".$user_name."' and id <> '".$id."'")->result())>0;
//            die (count($this->db->query("select * from users where username ='".$user_name."' and id <> '".$id."'")->result())>0);
        }
        function check_email_update($email, $id)
        {
            return count($this->db->query("select * from users where email ='".$email."' and id <> '".$id."'")->result())>0;
//            die(count($this->db->query("select * from users where email ='".$email."' and id <> '".$id."'")->result())>0);
        }
