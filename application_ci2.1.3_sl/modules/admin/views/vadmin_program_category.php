    ...
    <div class="grid_data">
        <div class="search_data">
            <select name="field_table" id="table_field">
                <option value="category_id" <?php echo $this->session->userdata("program_category_like_index")=="category_id" ? " selected " : "";?>>Id</option>
                <option value="category_name"<?php echo $this->session->userdata("program_category_like_index")=="category_name" ? " selected " : "";?>>Region</option>
            </select>
        ...
        <table id="program_category_data">
        </table>    
        <div id="pagersr"></div>
    </div>
...
{
    var request = $.ajax({
        url: '<?php echo site_url("admin/program_category/details");?>' ,
        type: "POST",
        data: 'category_id='+id,
        dataType: "json",
        beforeSend:function()
    ...
    $('<div></div>').appendTo('body')
                ...
                .dialog({
                    ...
                    buttons: {
                        Yes: function () {
                            var request = $.ajax({
                                url: '<?php echo site_url("admin/program_category/deleting");?>' ,
                                type: "POST",
                                data: 'category_id='+id,
                                dataType: "json",
                                beforeSend:function()
                            ...
                            request.done(function(msg) {
                                    if(msg.region_status = "success")
                                    {
                                        ...
                                        var url ='<?php echo site_url('admin/program_category/loadgrid');?>?like_index=' + $("#table_field").val() + '&like_value=' + $("#search_value").val() +'&where_index=&where_value=';
                                        firstload(url);
...
{
    $("#program_category_data").jqGrid('GridUnload');
    jQuery("#program_category_data").jqGrid({
            url:url,
            datatype: "xml",
            height: 300,
            width:1250,
...
//                alert(region_name.length);
                ...
                else
                {
                            ...
                            var request = $.ajax({
                                url: '<?php echo site_url("admin/program_category/insert");?>' ,
                                type: "POST",
...
//                alert(region_name.length);
                ...
                else
                {
                            ...
                            var request = $.ajax({
                                url: '<?php echo site_url("admin/program_category/update");?>' ,
                                type: "POST",
