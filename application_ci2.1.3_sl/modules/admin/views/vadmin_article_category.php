    ...
    <div class="grid_data">
        <div class="search_data">
            <select name="field_table" id="table_field">
                <option value="category_id" <?php echo $this->session->userdata("article_category_like_index")=="category_id" ? " selected " : "";?>>Id</option>
                <option value="category_name"<?php echo $this->session->userdata("article_category_like_index")=="category_name" ? " selected " : "";?>>Category</option>
            </select>
            <input type="text" id="search_value" value="<?php echo $this->session->userdata("article_category_like_value");?>"/>
        ...
        <table id="article_category_data">
        </table>    
        <div id="pagersr"></div>
    </div>
...
{
    var request = $.ajax({
        url: '<?php echo site_url("admin/article_category/details");?>' ,
        type: "POST",
        data: 'category_id='+id,
        dataType: "json",
        beforeSend:function()
    ...
    $('<div></div>').appendTo('body')
                ...
                .dialog({
                    ...
                    buttons: {
                        Yes: function () {
                            var request = $.ajax({
                                url: '<?php echo site_url("admin/article_category/deleting");?>' ,
                                type: "POST",
                                data: 'category_id='+id,
                                dataType: "json",
                                beforeSend:function()
                            ...
                            request.done(function(msg) {
                                    if(msg.region_status = "success")
                                    {
                                        ...
                                        var url ='<?php echo site_url('admin/article_category/loadgrid');?>?like_index=' + $("#table_field").val() + '&like_value=' + $("#search_value").val() +'&where_index=&where_value=';
                                        firstload(url);
...
{
    $("#article_category_data").jqGrid('GridUnload');
    jQuery("#article_category_data").jqGrid({
            url:url,
            datatype: "xml",
            height: 300,
            width:1250,
    ...
    jQuery("#article_category_data").jqGrid().bind("reloadGrid");
}
function search(url)
{
    alert("search");    
...
//                alert(region_name.length);
                ...
                else
                {
                            ...
                            var request = $.ajax({
                                url: '<?php echo site_url("admin/article_category/insert");?>' ,
                                type: "POST",
...
//                alert(region_name.length);
                ...
                else
                {
                            ...
                            var request = $.ajax({
                                url: '<?php echo site_url("admin/article_category/update");?>' ,
                                type: "POST",
