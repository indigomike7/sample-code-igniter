...
//        $this->lang->load('region');
        ...
        $this->tables  = array('university'=>'universities','course'=>'courses');

        //initialize data
        $this->identity_column = 'course_id';
        $this->join='course_id';
    ...
    /**
        ...
        * @author Ben Edmunds
        **/
    public function courses($limit, $offset, $order_by,$order,$where,$like_index,$like_value)
    {
        $this->_limit = $limit;
        $this->_offset=$offset;
...
//           die ($this->_limit." x y". $this->_offset);
        ...
        $this->db->join('universities', 'universities.university_id = courses.university_id','left');
        $this->response = $this->db->get($this->tables['course']);
//        echo $this->db->last_query();

        return $this;
    }
    public function count_courses($where)
    {
        
        $this->trigger_events('count_courses');
        $this->db->select(array(
            'count(1) as count'
            ));
        $this->_where=$where;
...
//        die($this->db->last_query());
    ...
    {
        $this->response = $this->db->query("delete from courses where course_id = '".$id."'");
        return $this;
    }
    public function insert($course_name, $course_description, $university_id)
    {
    ...
    public function update($course_name, $course_description, $university_id, $course_id)
    {
        $this->response = $this->db->query("
            update courses set 
    ...
    public function lecturer_in_course($course_id)
    {
        $this->response = $this->db->query("
            select courses_lecturer.*,
            courses.course_name
            from courses_lecturer 
            left join courses 
            on courses.course_id = courses_lecturer.course_id
            where courses_lecturer.course_id = '".$course_id."'
            ");
        return $this;
    }
    public function insert_cl($lecturer_name, $course_id)
    {
        $this->response = $this->db->query("
            insert into courses_lecturer( 
            lecturer_name
            , course_id
            ) 
            values
    ...
    public function delete_cl($cl_id)
    {
        $this->response = $this->db->query("
            delete from courses_lecturer 
                where cl_id = '".$cl_id."'"
                );
        return $this;
    }
